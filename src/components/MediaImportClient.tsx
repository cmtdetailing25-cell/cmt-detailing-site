...
export default function MediaImportClient() {
  ...
  // Load clients on mount; handle URL-param deep-links
  useEffect(() => {
  ...
  // Auto-fill job title when vehicle + service type are chosen
  useEffect(() => {
  ...
  async function handleClientNext() {
    ...
    try {
      if (isNewClient) {
        if (!newClientForm.firstName.trim() || !newClientForm.lastName.trim()) {
  ...
  async function handleUpload() {
    ...
    // Upload phase — re-read files state after compression
    setFiles((prev) => {
      const toUpload = prev.filter((f) => ["compressed", "already-optimized"].includes(f.status));
      // kick off uploads asynchronously
      (async () => {
        for (const entry of toUpload) {
          setFiles((p) => p.map((f) => f.id === entry.id ? { ...f, status: "uploading" } : f));
          try {
            ...
            setFiles((p) => p.map((f) => f.id === entry.id ? { ...f, status: "uploaded", result: data } : f));
          } catch (err) {
            const msg = err instanceof Error ? err.message : "Upload failed";
            setFiles((p) => p.map((f) => f.id === entry.id ? { ...f, status: "failed", error: msg } : f));
          }
        }
        setUploading(false);
        if (toUpload.length > 0) setAllDone(true);
      })();
      return prev;
    });
  }
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-white mb-1">Import Photos</h1>
      <p className="text-gray-400 text-sm mb-6">Assign photos to a client, vehicle, and detail job before uploading.</p>
      ...
      {step === "vehicle" && (
        <div>
          ...
          {isNewVehicle && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-4 space-y-3">
              <div className="grid grid-cols-3 gap-3">
                {(["year", "make", "model"] as const).map((key) => (
                  <div key={key}>
                    <label className="block text-[11px] text-gray-500 mb-1 capitalize">{key} *</label>
                    <input
                      value={newVehicleForm[key]}
                      onChange={(e) => setNewVehicleForm((f) => ({ ...f, [key]: e.target.value }))}
                      placeholder={key === "year" ? "2022" : key === "make" ? "BMW" : "X5"}
                      className="w-full bg-gray-800 border border-gray-700 rounded px-2.5 py-1.5 text-sm text-white focus:outline-none focus:border-[#94b2b6]"
                    />
                  </div>
                ))}
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={newVehicleForm.isPrimary}
                  onChange={(e) => setNewVehicleForm((f) => ({ ...f, isPrimary: e.target.checked }))}
                  className="rounded"
                />
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setStep("client")} className="text-sm text-gray-600 hover:text-gray-400 transition-colors">
              ← Back
            </button>
          </div>
        </div>
      ...
      {step === "job" && (
        <div>
          ...
          {isNewJob && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-4 space-y-3">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={newJobForm.isSocialReady}
                  onChange={(e) => setNewJobForm((f) => ({ ...f, isSocialReady: e.target.checked }))}
                  className="rounded"
                />
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setStep("vehicle")} className="text-sm text-gray-600 hover:text-gray-400 transition-colors">
              ← Back
            </button>
          </div>
        </div>
